...
```sh
code --install-extension poly-syntax-0.4.2.vsix
code --install-extension poly-lint-darwin-arm64-0.4.2.vsix
```
...
```sh
POLY_VERSION=0.4.2 POLY_INSTALL_DIR=~/bin sh install.sh
```
...
`@v0` 會跟著最新的 release 走。要釘死版本就寫 `with: { version: "0.4.2" }`——poly
...
`linux/amd64` 與 `linux/arm64` 都有。tag 有 `latest`、`0.4.2`、`0.4`；pre-release
