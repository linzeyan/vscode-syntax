...
> poly-syntax-0.8.0.vsix). The VSIX files were downloaded — install them
...
```sh
POLY_VERSION=0.8.0 POLY_INSTALL_DIR=~/bin sh install.sh
```
...
`@v0` 會跟著最新的 release 走。要釘死版本就寫 `with: { version: "0.8.0" }`——poly
...
`linux/amd64` 與 `linux/arm64` 都有。tag 有 `latest`、`0.8.0`、`0.8`；pre-release
