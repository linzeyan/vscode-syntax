...
- **跨檔案 next／previous change ＋ `Poly: Revert Selected Changes and Save`**：
  ...
  按鍵按兩次一定走同一條路。別的擴充綁了同一組鍵（Rewrap 的 `alt+q`）時 poly 讓出來。
...
```sh
code --install-extension poly-syntax-highlight-0.18.7.vsix
code --install-extension poly-lsp-darwin-arm64-0.18.7.vsix
```
...
> poly-syntax-0.18.7.vsix). The VSIX files were downloaded — install them
...
```sh
POLY_VERSION=0.18.7 POLY_INSTALL_DIR=~/bin sh install.sh
```
...
`@v0` 會跟著最新的 release 走。要釘死版本就寫 `with: { version: "0.18.7" }`——poly
...
`linux/amd64` 與 `linux/arm64` 都有。tag 有 `latest`、`0.18.7`、`0.18`；pre-release
