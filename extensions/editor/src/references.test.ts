...
test("a file's declarations and the methods on them get a lens", () => {
  ...
  assert.deepEqual(
    lensTargets(file, 100).map((s) => s.name),
...
test("a local inside a function gets nothing", () => {
  ...
  assert.deepEqual(lensTargets(file, 100).map((s) => s.name), ["main", "out"]);
});
...
test("fields are not counted", () => {
  ...
  assert.deepEqual(lensTargets(file, 100).map((s) => s.name), ["Config"]);
});
...
test("the declaration itself is not one of its references", () => {
  ...
  assert.equal(countReferences(locations, declaration), 2);
  assert.equal(countReferences([declaration], declaration), 0);
  // Same line number in a different file is a different place.
  assert.equal(
    countReferences([{ uri: "file:///p/other.go", line: 9 }], declaration),
    1,
  );
});
